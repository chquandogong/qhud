...
<!-- qhud:anchor -->
<a id="d-021--ambient-diagnostics-are-typed-and-redacted"></a>

## D-021 · 평상시 진단 로그는 형식화하고 비식별화

- **맥락**: CodeQL 경고에서 Codex 범위 불일치 경로가 안정적인 계정 ID를 stderr에 기록한다는 사실을 찾았습니다. 인접 경로 감사에서는 평상시 진단에 자격 증명 파일명, 공급자 본문 미리보기, 계정·창 레이블, 확장된 설정 경로, 사용량 값, JavaScript 예외 원문도 있었습니다. 두 번째 발견으로 D-019의 serde 타입 오류가 항상 식별 정보가 없다는 가정도 틀렸음이 확인됐습니다. 예상하지 못한 문자열은 원본 오류에 인용됩니다.
- **결정**: 평상시 진단은 형식과 범위가 정해진 사실만 받습니다. UI 이벤트는 enum과 정수 카운터로 Rust 경계를 통과하며 출처 레이블, 계정·작업공간 ID, 경로, 백분율, 예외 원문, 공급자 값은 평상시 로그에 들어가지 않습니다. 공급자 JSON 오류는 거친 범주와 위치만 보고합니다. 운영자가 명시적으로 실행한 JSON 명령은 전체 진단 출력을 유지하되 공유 전 민감 정보 제거가 필요함을 문서화합니다.
- **기각**: 식별자 해시(안정적인 해시는 계속 연결 가능한 가명임), 상호작용 진단 삭제(D-010은 포인터 전달과 렌더링 형태의 증거가 필요함), 원본 serde/RPC 오류 기록(상위 문자열은 신뢰할 수 없는 데이터임).
- **증거**: 이메일, 토큰, 경로, 계정, 작업공간 canary가 형식 변환 테스트에서 거부되거나 결과에 없음을 확인합니다. 저장소 검사, Linux/Windows CI, CodeQL, 독립 로그 흐름 검토를 변경 전체의 필수 게이트로 둡니다.
