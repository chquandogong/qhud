...
- **背景**：CodeQL 告警发现 Codex 范围不匹配路径把稳定账户 ID 写入 stderr。相邻路径审计还发现日常诊断中存在凭据文件名、供应商响应预览、账户与窗格标签、展开后的配置路径、用量值及 JavaScript 异常原文。第二项发现推翻了 D-019 中 serde 类型错误必然不含身份的假设：意外字符串会被原始错误引用。
- **决策**：日常诊断只接受类型化且有界的事实。UI 事件通过枚举和整数计数器跨越 Rust 边界；来源标签、账户/工作区 ID、路径、百分比、异常原文和供应商值都不得进入日常日志。供应商 JSON 失败只报告粗粒度类别与位置。操作者明确执行的 JSON 命令保留完整诊断输出，并明确说明分享前须删除敏感信息。
- **否决**：对标识符做哈希，因为稳定哈希仍是可关联假名；删除交互诊断，因为 D-010 仍需要指针传递与渲染结构的证据；记录原始 serde 或 RPC 错误，因为上游字符串是不受信任的数据。
- **证据**：格式化测试确认邮箱、令牌、路径、账户和工作区 canary 被拒绝或不出现在输出中。仓库检查、Linux/Windows CI、CodeQL 及独立日志流审查被设为完整变更的必过门槛。
